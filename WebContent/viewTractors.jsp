...
<div class="row">
		<div class="col-md-8">
			<h4 style="color:green" ><font><strong>Tractors details</strong></font></h4>
		</div>
	...
	<div>
			...
			<%
				try{
					...
					if(search != null){
						query = "select * from tractors where number_plate like '%"+search+"%' or insurance_company like '%"+search+"%' or premium like '%"+search+"%' or vehicle_condition like '%"+search+"%' or purchase_date like '%"+search+"%' or assigned_driver like '%"+search+"%'";
					} else {
					...
					while(rs.next()){
						...
						<tbody>
							...
							<td class="text-right">
								...
								<button name="delete" id="<%= rs.getInt("id")%>" data-toggle="modal" data-target="#delete_modal<%= rs.getInt("id")%>" class="btn btn-danger" style="width:80px;" >Delete</button>
							</td>
						</tr>
							...
							<div class="modal-body">
								...
								<div class="form-group">
									...
									<select id="condition" name="condition" class="form-control" value='<%=rs.getString("vehicle_condition") %>'>
										<option selected><%= rs.getString("vehicle_condition")%></option>
										<option>New</option>
										 <option>Good</option>
										 <option>Fair</option>
										 <option>Bad</option>
				...
				<div class="modal modal-danger " id="delete_modal<%= rs.getInt("id")%>" tabindex="-1">
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body">
							<form action="DeleteTractor" method="POST">
								<input type="hidden" name="id" value='<%=rs.getInt("id") %>'/>
								<div class="form-group">
