...
/**
 * Servlet implementation class EditTractor
 */
@WebServlet("/EditTractor")
public class EditTractor extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(number_plate.equals("") || insurance_company.equals("") || premium < 0 || condition.equals("") || purchase_date.equals(""))  {
			...
			request.getRequestDispatcher("viewTractors.jsp").forward(request, response);
		} else{
		
			DAOHandler editTractorDao = new DAOHandler();
			int result = editTractorDao.edit_tractor(id, number_plate, insurance_company, premium, condition, purchase_date, assigned_driver);
			if(result > 0){
